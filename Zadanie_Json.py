...
def get_data_posts(url_posts):  # loads json from url, then creates the dictionary with post_data which I will be using lately
    response_posts = urllib.request.urlopen(url_posts)
    data_posts = json.loads(response_posts.read())
    return data_posts


def get_data_users(url_users):  # loads json from url, then creates the dictionary with user data which I will be using lately
...
data_posts = get_data_posts(url_posts)
data_users = get_data_users(url_users)
# task 2
number = number_of_posts(data_posts, data_users)
for i in range(0, len(data_users)):
    print(data_users[i]['name'] + ' napisal(a) ' + str(number[i]) + ' postow!')
print('\n')
# task 3
...
print('\n')
# task 4
closest = closest_neighbour(data_users)
for i in range(0, len(data_users)):
    print(data_users[i]['name'] + ' is closest to ' + data_users[closest[i]]['name'])
    print('UserId ' + str(i + 1) + ' is closest to ' + 'userId ' + str(closest[i] + 1))
    print('')
